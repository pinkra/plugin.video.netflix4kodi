...
s = requests.Session()
#r = s.get('https://www.netflix.com/browse/genre/83')
#print r.text

r = s.post('https://www.netflix.com/Login?locale=it-IT');

matches = re.findall('authURL.*value="(.+)"', r.text)
authURL=matches[0]

payload = {'email': '<email>', 'password': '<pass>', 'authURL': authURL}
cookies = dict(NetflixCookies='true',memclid=r.cookies['memclid'])
r2 = s.post('https://www.netflix.com/Login?locale=it-IT', data=payload, cookies=cookies);

cookies = dict(profilesNewUser='0',profilesNewSession='0',dsca='anonymous',memclid=r2.cookies['memclid'])

nflxData = s.get('https://www.netflix.com/browse/genre/83', cookies=cookies).text.encode('utf8');
...
def get_videos(category):
    ...
    props = re.findall('(netflix\.[A-z]+)\s*=', nflxData)

    for i in props:
      if (i == 'netflix.falkorCache'):
        chunk = re.findall(i + '\s*=\s*({.*?});</script>', nflxData)
        if (len(chunk) > 0):
          data = json.loads(chunk[0])
          items = data['videos']
          for i in items:
            if isinstance(items[i], dict) and items[i].has_key('title') and isinstance(items[i]['title'], dict):
              vid = { 'name': items[i]['title']['value'],'genre':None,'thumb':None,'video':None}
              if isinstance(items[i]['boxarts'], dict):
                for p in items[i]['boxarts']:
                  if re.search('\d+x\d+', p):
                    vid['thumb'] = items[i]['boxarts'][p]['jpg']['url']
              vids.append(vid)
